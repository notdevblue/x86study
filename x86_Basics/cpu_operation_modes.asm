...
; 현대 32비트 OS 에서 프로그래밍 하는 경우 (Linux나 Windows같은), 기본적으로 flat 32-bit 모드로 프로그래밍 하는 것임.
; 주소를 지정하는데 아무 레지스터나 사용할수 있고, 32비트 레지스터 전부를 사용하는 것이 16비트 레지스터를 사용하는 것 보다 효율적임.
; 추가적으로 세그먼트 레지스터는 일반적으로 플랫 모드에서 사용되지 않고, 이를 사용하는건 좋지 못한 사례임.

; ### Muti-Segmented Memory Model

; 32비트 레지스터로 메모리를 접근하는 경우, 현대 컴퓨터의 프로그램은 (거의) 모든 메모리를 접근할 수 있음.
; 이전의 프로세서들 (16비트 레지스터만 가진) 에선 Segmented memory model 이 사용됨. 'CS', 'DS', 'ES' 레지스터가 메모리의 각기 다른 청크를 가리켰음.
; 작은 프로그램(작은 모델) 인 경우 CS=DS=ES 였음. 큰 메모리 모델인 경우, 세그먼트들은 각기 다른 위치를 가리켰음.
